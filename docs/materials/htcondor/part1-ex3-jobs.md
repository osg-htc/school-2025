...
When you want to run an HTCondor job, you will first need to write an HTCondor submit file for it. In this exercise, you will run the same `hostname` command as in Exercise 1.1, but this time, the command will run within a job on one of the *execution points* (aka a machine) in the OSPool, instead of the access point.
...
| `request_*`     | How many `cpus`, `memory`, and `disk` we want. In this case, we don't need much because the 'hostname' executable is very small.                               |
| `requirements`     | Other requirements for our jobs. In this example, we request machines with the Red Hat Enterprise Linux 9 operating system.                              |
...
Most likely, you will want to run a program that exists within your directory 
...
1. Test your script from the command line:
        ...
        Date:  Thu May 1 10:21:54 AM CDT 2025
        Host:  ospool-ap2140
    ...
    If you cannot run your executable from the command line, HTCondor probably cannot run it on another machine. It's important to test your code before you submit it as a job, but be aware that you should *never* run compute-heavy code directly on the access point.
